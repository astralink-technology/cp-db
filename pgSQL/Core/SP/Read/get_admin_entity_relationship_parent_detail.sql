...
DECLARE fname text;
BEGIN
FOR fname IN SELECT oid::regprocedure FROM pg_catalog.pg_proc WHERE proname = 'get_admin_entity_relationship_parent_detail' LOOP
  EXECUTE 'DROP FUNCTION ' || fname;
END loop;
RAISE INFO 'FUNCTION % DROPPED', fname;
END$$;
-- Start function
CREATE FUNCTION get_admin_entity_relationship_parent_detail(
...
BEGIN
    ...
    entity_relationship er ON er.related_id = e.entity_id
    WHERE (
    ...
    CREATE TEMP TABLE admin_entity_relationship_parent_init AS
      SELECT
        e.entity_id
        , er.entity_id AS parent_id
        , e.first_name
        , e.last_name
        , e.nick_name
        , e.name
      ...
      entity_relationship er ON er.related_id = e.entity_id WHERE (
      ((pEntityId IS NULL) OR (e.entity_id = pEntityId)) AND
    ...
    SELECT
      *
      , totalRows
    FROM admin_entity_relationship_parent_init;
