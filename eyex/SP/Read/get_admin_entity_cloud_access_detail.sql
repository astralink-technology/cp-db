...
DECLARE fname text;
BEGIN
FOR fname IN SELECT oid::regprocedure FROM pg_catalog.pg_proc WHERE proname = 'get_admin_entity_cloud_access_detail' LOOP
  EXECUTE 'DROP FUNCTION ' || fname;
END loop;
RAISE INFO 'FUNCTION % DROPPED', fname;
END$$;
-- Start function
CREATE FUNCTION get_admin_entity_cloud_access_detail(
...
BEGIN
    ...
    authentication a ON a.authentication_id = e.authentication_id LEFT JOIN
     ...
     cloud_access ca ON ca.owner_id = e.entity_id;
    ...
    CREATE TEMP TABLE admin_entity_init AS
      ...
      address ad ON ad.owner_id = e.entity_id LEFT JOIN
      phone p ON p.phone_id = e.primary_phone_id LEFT JOIN
      cloud_access ca ON ca.owner_id = e.entity_id WHERE (
