...
DECLARE fname text;
BEGIN
FOR fname IN SELECT oid::regprocedure FROM pg_catalog.pg_proc WHERE proname = 'get_dependent_details' LOOP
  EXECUTE 'DROP FUNCTION ' || fname;
END loop;
RAISE INFO 'FUNCTION % DROPPED', fname;
END$$;
-- Start function
CREATE FUNCTION get_dependent_details(
...
RETURNS TABLE(
    ...
    , related_entity_device_id varchar(32)
    , total_rows integer
  )
AS
$BODY$
...
BEGIN
    ...
    CREATE TEMP TABLE entity_relationship_init AS
       SELECT
        ...
        , dr.device_id as related_entity_device_id
          ...
          authentication eea ON eea.authentication_id = ee.authentication_id LEFT JOIN
          device_relationship dr ON dr.owner_id = ee.entity_id WHERE (
