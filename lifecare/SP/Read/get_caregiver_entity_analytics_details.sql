...
DECLARE fname text;
BEGIN
FOR fname IN SELECT oid::regprocedure FROM pg_catalog.pg_proc WHERE proname = 'get_caregiver_entity_analytics_details' LOOP
  EXECUTE 'DROP FUNCTION ' || fname;
END loop;
RAISE INFO 'FUNCTION % DROPPED', fname;
END$$;
-- Start function
CREATE FUNCTION get_caregiver_entity_analytics_details(
        pCaregiveEntityId varchar(32)
        , pPageSize integer
        , pSkipSize integer
    )
RETURNS TABLE(
...
BEGIN
    ...
    CREATE TEMP TABLE caregiver_entity_device_relationship_init AS
      SELECT
        e.entity_id
        , e.first_name
        , e.last_name
    ...
    SELECT
      *
      , totalRows
    FROM caregiver_entity_device_relationship_init;
