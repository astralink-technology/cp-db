...
DECLARE fname text;
BEGIN
FOR fname IN SELECT oid::regprocedure FROM pg_catalog.pg_proc WHERE proname = 'get_median_wakeup_time' LOOP
  EXECUTE 'DROP FUNCTION ' || fname;
END loop;
RAISE INFO 'FUNCTION % DROPPED', fname;
END$$;
-- Start function
CREATE FUNCTION get_median_wakeup_time(
      pOwnerId varchar(32)
      , pDay date
    )
RETURNS TABLE(
...
BEGIN
    RETURN QUERY
      ...
      SELECT
        ia.date_value2 AS median_wakeup_time
