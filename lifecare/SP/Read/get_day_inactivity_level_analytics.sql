...
DECLARE fname text;
BEGIN
FOR fname IN SELECT oid::regprocedure FROM pg_catalog.pg_proc WHERE proname = 'get_day_inactivity_level_analytics' LOOP
  EXECUTE 'DROP FUNCTION ' || fname;
END loop;
RAISE INFO 'FUNCTION % DROPPED', fname;
END$$;
-- Start function
CREATE FUNCTION get_day_inactivity_level_analytics(
...
BEGIN
 ...
 -- get the wake up time
  ...
  -- Day inactivity level table init
  CREATE TEMP TABLE IF NOT EXISTS day_inactivity_level_init(
      eyecare_id varchar(32)
      , inactivity_duration integer
      , next_row_create_date timestamp without time zone
      , prev_row_create_date timestamp without time zone
  ...
  DELETE FROM day_inactivity_level_init;

  IF pDayOverlaps = true THEN
    INSERT INTO day_inactivity_level_init (eyecare_id, inactivity_duration, next_row_create_date, prev_row_create_date, create_date)
      SELECT
        ee.*
       FROM(
          SELECT
      ...
      ((ee.create_date BETWEEN (pDay || ' ' ||  '00:00:00')::timestamp AND pSleepingTime) OR
      (ee.create_date BETWEEN pWakeupTime AND (pDay || ' ' || '23:59:59')::timestamp));
  ...
  ELSEIF pNightOverlaps = true THEN
    ...
    INSERT INTO day_inactivity_level_init (eyecare_id, inactivity_duration, next_row_create_date, prev_row_create_date, create_date)
      ...
      ee.next_row_create_date NOT IN (SELECT * FROM away_start_end_timings_init) AND
      ee.prev_row_create_date NOT IN (SELECT * FROM away_start_end_timings_init) AND
      (ee.create_date BETWEEN pWakeupTime AND pSleepingTime);
  ELSE
      INSERT INTO day_inactivity_level_init (eyecare_id, inactivity_duration, next_row_create_date, prev_row_create_date, create_date)
        SELECT
          ee.*
    ...
    RETURN QUERY
      SELECT
        *
        , pWakeupTime
        , pSleepingTime
      FROM day_inactivity_level_init;
