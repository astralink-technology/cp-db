...
DECLARE fname text;
BEGIN
FOR fname IN SELECT oid::regprocedure FROM pg_catalog.pg_proc WHERE proname = 'get_entity_core_analytics_details' LOOP
  EXECUTE 'DROP FUNCTION ' || fname;
END loop;
RAISE INFO 'FUNCTION % DROPPED', fname;
END$$;
-- Start function
CREATE FUNCTION get_entity_core_analytics_details(
...
BEGIN
    ...
    CREATE TEMP TABLE admin_entity_core_analytics_init AS
      SELECT
        ...
        , ad.longitude
        , ad.latitude
        , (SELECT ca.date_value FROM core_analytics ca WHERE ca.type = 'MW' ORDER BY ca.date_value desc LIMIT 1) as median_wakeup_time
        , (SELECT ca.date_value FROM core_analytics ca WHERE ca.type = 'MS' ORDER BY ca.date_value desc LIMIT 1) as median_sleeping_time
        -- some of the values could not be global, requires to be user specific
    ...
    SELECT
      *
      , totalRows
    FROM admin_entity_core_analytics_init;
